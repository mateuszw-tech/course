...
def calculate(a):
    ...
    if num_one.isdigit() and num_two.isdigit():
        if a == 'add_button':
            result = calculator.Calculator(int(num_one), int(num_two)).add()
        elif a == 'sub_button':
            result = calculator.Calculator(int(num_one), int(num_two)).subtract()
        elif a == 'mul_button':
            result = calculator.Calculator(int(num_one), int(num_two)).multiply()
        elif a == 'div_button':
            result = calculator.Calculator(int(num_one), int(num_two)).divide()
        elif a == 'pow_button':
            result = calculator.Calculator(int(num_one), int(num_two)).power()
        else:
            result = 0
    else:
